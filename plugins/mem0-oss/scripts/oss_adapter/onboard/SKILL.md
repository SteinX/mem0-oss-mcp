...
2. If tools are missing, inspect the generated `.mcp.json`. In the recommended stdio mode, confirm it points to `scripts/mem0_oss_stdio_bridge.py` and that `MEM0_OSS_ENV_FILE` exists. In direct HTTP mode, explain that the token env var named by the generated plugin must be present in the Codex process environment, then start a new thread.
